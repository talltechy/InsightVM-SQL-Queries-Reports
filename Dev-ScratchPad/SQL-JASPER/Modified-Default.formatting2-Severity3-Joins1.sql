WITH
   ...
   asset_metadata AS (
      SELECT da.asset_id
      ,ds.name AS primary_site_name
      ,da.ip_address AS ip_address
      ,da.host_name
      ,da.sites
      FROM dim_asset da
         LEFT JOIN dim_site_asset dsa ON dsa.asset_id = da.asset_id
         LEFT JOIN dim_site ds ON ds.site_id = dsa.site_id
   )
SELECT vfa.asset_id
,am.primary_site_name AS "Primary Site"
...
LEFT JOIN asset_metadata am ON am.asset_id = vfa.asset_id
ORDER BY primary_site_name, ip_address, vulnerability_title