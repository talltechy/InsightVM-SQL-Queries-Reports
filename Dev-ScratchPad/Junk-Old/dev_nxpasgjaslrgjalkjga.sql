WITH
    vuln_references AS (
        ...
        LEFT JOIN fact_asset_vulnerability_age fava ON dv.vulnerability_id = fava.vulnerability_id
        GROUP BY dv.vulnerability_id, fava.age_in_days, dv.severity, dv.riskscore, dv.nexpose_id, dv.title, dv.description, dv.exploits, dv.malware_kits
    ),
    dfa_assets AS (
        SELECT da.asset_id
        ...
        GROUP BY da.asset_id, da.ip_address,da.mac_address,da.host_name, fa.riskscore, fa.critical_vulnerabilities, fa.severe_vulnerabilities, fa.moderate_vulnerabilities, fa.vulnerabilities, daga.asset_group_id
    )
--begin SELECT
